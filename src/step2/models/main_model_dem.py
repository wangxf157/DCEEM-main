...
class Main_Model_Dem(BaseModel):
    ...
    def initialize(self, opt):
		...
		if opt.which_model_netG == 'introAE':
			...
			self.netG = IntroAE(norm=opt.norm, gpuId=opt.gpu_ids, cdim=3, hdim=opt.hdim,
								channels=str_to_list(opt.channels), image_size=opt.output_height)
			self.encoder2 = NCEEncoder(norm=opt.norm, cdim=3, hdim=opt.hdim, channels=str_to_list(opt.channels),
									   image_size=opt.output_height)
	...
	def validation(self):
		if self.opt.which_model_netG == 'introAE':
			with torch.no_grad():
				...
				self.netG.eval()
				self.encoder2.eval()
				self.latent_t, self.fake_B, self.loss_vq = self.netG(self.real_B)
				quantized_latent, x_recon, loss_vq, self.encodings = self.netG.encoder(self.real_B)

				self.latent_t = self.latent_t.detach()
				self.fake_B = self.fake_B.detach()
				self.latent_f, self.latent_i = self.encoder2(self.real_A, self.netG.encoder._vq_vae)

				self.fake_Bi = self.netG.decoder(self.latent_f)

				self.latent_otLoss = self.contentLoss.get_l1loss(self.latent_i,self.latent_t)
	...
	def backward_G_E2(self):
		if self.opt.which_model_netG == 'introAE':
			...
			self.latent_otLoss = self.contentLoss.get_l1loss(self.latent_i,self.latent_t)
			self.loss_G_Content = self.contentLoss.get_mseloss(self.fake_Bi, self.real_B)
			# self.loss_G_Content = self.contentLoss.get_mseloss(self.fake_Bi, self.real_B)*0.0003

			# self.loss_E1D = self.latent_otLoss*0.0001 + self.loss_G_Content
			weight1 = self.loss_G_Content / (self.latent_otLoss + self.loss_G_Content)
			weight2 = self.latent_otLoss / (self.latent_otLoss + self.loss_G_Content)
			self.loss_E1D = self.latent_otLoss*weight1 + self.loss_G_Content*weight2
	...
	def get_current_errors(self):
		return OrderedDict([('G_otlatent No', self.latent_otLoss.item()),
                            ('G_loss_G_Content No', self.loss_G_Content.item()),
							('G_loss_E No', self.loss_E1D.item()),
							])
	...
	def get_current_errors_val(self):
		return OrderedDict([('G_otlatent_val', self.latent_otLoss.item()),
							...
							('G_nceloss_val', self.loss_G_Content.item())
						])
