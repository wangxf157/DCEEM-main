...
class Main_model(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.which_model_netG == 'introAE':
            ...
            self.optimizer_G_E1 = torch.optim.Adam(self.netG.encoder.parameters(), lr=opt.lr)
            self.optimizer_G_D = torch.optim.Adam(self.netG.decoder.parameters(), lr=opt.lr)
    ...
    def validation(self):
        if self.opt.which_model_netG == 'introAE':
            with torch.no_grad():
                ...
                self.netG.eval()
                self.latent, self.fake_B, self.loss_vq = self.netG(self.real_B)
    ...
    def forward(self):
        ...
        if self.opt.which_model_netG == 'introAE':
            self.latent, self.fake_B, self.loss_vq = self.netG(self.real_A)
        elif self.opt.which_model_netG == 'vqgan':
            self.latent, self.fake_B, self.loss_vq, _ = self.netG(self.real_A)
        else:
            raise ValueError('This repo only support the autoencoder modified from introAE, i.e., opt.which_model_netG == introAE. \
            But you can use this option to add new model')
    ...
    def optimize_parameters(self):
        if self.opt.which_model_netG == 'introAE':
            ...
            self.loss_G_L1 = self.contentLoss.get_l1loss(self.fake_B, self.real_B)
            self.loss_G_L2 = self.contentLoss.get_mseloss(self.fake_B, self.real_B)
            self.loss_G = self.loss_vq*6000 + self.loss_G_L2*0.001 	# only use L1 and perceptual loss  20 0.001 5
    ...
    def get_current_errors(self):
        if self.opt.which_model_netG == 'introAE':
            return OrderedDict([('G_VQ*6000', self.loss_vq.item()*6000),
                                ('G_percetual No', self.loss_G_Content.item()),
                                ('G_L1', self.loss_G_L1.item()),
                                ('G_L2', self.loss_G_L2.item()*0.001)
                                ])
        elif self.opt.which_model_netG == 'vqgan':
            return OrderedDict([('G_Adv', self.loss_adv),
                                ('G_Recon', self.loss_recon),
                                ('G_VQ', self.loss_vq),
                                ('G_Total', self.loss_G),
                                ('D_Total', self.loss_D),
                                ('G_Perceptual', self.loss_G_Content.item()),
                                ('G_L1', self.loss_G_L1.item()),
    ...
    def save(self, label):
        if self.opt.which_model_netG == 'introAE':
            ...
            self.save_network(self.netG.decoder, 'G_Decoder', label, self.gpu_ids)
            self.save_network(self.netG.encoder, 'G_Encoder1', label, self.gpu_ids)
            self.save_network(self.netG.discriminator, 'D', label, self.gpu_ids)
        else:
            raise ValueError('This repo only support the autoencoder modified from introAE, i.e., opt.which_model_netG == introAE. \
            But you can use this option to add new model')
    ...
    def update_learning_rate(self):
        ...
        if self.opt.which_model_netG == 'introAE':
            ...
            for param_group in self.vqgan_trainer.optimizer_G.param_groups:
                param_group['lr'] = lr
            for param_group in self.vqgan_trainer.optimizer_D.param_groups:
                param_group['lr'] = lr
